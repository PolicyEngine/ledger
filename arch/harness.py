"""Fixture-first Arch fact validation harness."""
...
from arch.bundle import BuildBundleReport, build_bundle
...
from arch.core import AggregateFact, ValidationReport, validate_facts
from arch.database import ArchDbBuildReport, build_arch_db
from arch.mirror import (
    ArchMirrorExportReport,
    SupabaseMirrorLoadReport,
    export_arch_db_tables,
    load_supabase_mirror,
)
from arch.pe_source_plan import (
    PeSourcePlanReport,
...
from arch.store import load_facts_jsonl, save_facts_jsonl
...
FIXTURE_BUILDERS = {
    "soi-table-1-1": "arch.jurisdictions.us.soi",
    "soi-table-1-4": "arch.jurisdictions.us.soi",
}
...
def build_fixture_source_rows(source: str, *, year: int) -> list[SourceRow]:
    ...
    package = None
    try:
        from arch.source_package import load_source_package
...
def build_arch_db_file(
    ...
    *,
    ...
    """Build a relational Arch DB from fact and source-cell JSONL files."""
...
def export_consumer_fact_file(
    ...
    *,
    ...
    """Export Arch facts to downstream consumer-contract JSONL."""
    facts = load_facts_jsonl(fact_path)
    validation_report = validate_facts(facts)
    if not validation_report.valid:
        raise ValueError("Cannot export invalid Arch facts to consumer contract.")
    contract_report = validate_consumer_fact_contract(facts)
    if not contract_report.valid:
        raise ValueError("Cannot export invalid Arch consumer-contract facts.")
...
def build_bundle_dir(
    ...
    *,
    ...
    """Build a merged Arch consumer bundle from source-package suites."""
    return build_bundle(
        output_dir,
        year=year,
        sources=sources,
...
def bootstrap_r2_storage(
    *,
    raw_bucket: str = "arch-raw",
    derived_bucket: str = "arch-derived",
    wrangler_command: str = "npx wrangler",
) -> R2BootstrapReport:
    """Create Arch R2 buckets when Wrangler is authenticated."""
...
def export_arch_db_table_files(
    db_path: str | Path,
    output_dir: str | Path,
    *,
    replace: bool = False,
) -> ArchMirrorExportReport:
    """Export local Arch DB tables to bulk-loadable JSONL files."""
    return export_arch_db_tables(db_path, output_dir, replace=replace)
...
def main(argv: list[str] | None = None) -> int:
    """Run the harness CLI."""
    parser = argparse.ArgumentParser(description="Arch fact validation harness")
    subparsers = parser.add_subparsers(dest="command", required=True)

    validate_parser = subparsers.add_parser(
        "validate-facts",
        help="Validate an Arch fact JSONL file",
    )
    input_group = validate_parser.add_mutually_exclusive_group()
    input_group.add_argument(
        "--input",
        type=Path,
        help="Path to an Arch fact JSONL file",
    )
    input_group.add_argument(
        "--fixture",
        action="store_true",
    ...
    source_row_validate_parser = subparsers.add_parser(
        ...
        help="Validate an Arch source-row JSONL file",
    )
    source_row_validate_parser.add_argument(
        "--input",
        type=Path,
        required=True,
        help="Path to an Arch source-row JSONL file",
    )

    build_parser = subparsers.add_parser(
        "build-fixture-facts",
        help="Build a tiny source-backed Arch fact JSONL file",
    )
    ...
    source_build_parser = subparsers.add_parser(
        "build-source-cells",
        help="Build whole-artifact Arch source-cell JSONL records",
    )
    ...
    source_row_build_parser = subparsers.add_parser(
        "build-source-rows",
        help="Build full-document Arch source-row JSONL records",
    )
    ...
    db_parser = subparsers.add_parser(
        "build-db",
        help="Build a relational Arch DB artifact",
    )
    ...
    db_parser.add_argument(
        ...
        type=Path,
        required=True,
        help="SQLite Arch DB path to write",
    )
    db_parser.add_argument(
        "--replace",
        action="store_true",
    ...
    consumer_parser = subparsers.add_parser(
        ...
        help="Export Arch facts to downstream consumer-contract JSONL",
    )
    ...
    suite_parser.add_argument(
        "--axiom-cli",
        help=(
            "Optional Axiom command, e.g. `axiom` or "
            "`uv run --with-editable . axiom`."
        ),
    )
    suite_parser.add_argument(
        "--axiom-root",
    ...
    package_validate_parser = subparsers.add_parser(
        "validate-package",
        help="Validate a declarative Arch source package",
    )
    ...
    raw_publish_parser = subparsers.add_parser(
        "publish-raw",
        help="Upload manifest-declared raw source artifacts to arch-raw R2",
    )
    ...
    r2_parser = subparsers.add_parser(
        "bootstrap-r2",
        help="Create Arch R2 buckets when Wrangler is authenticated",
    )
    ...
    r2_parser.add_argument(
        "--derived-bucket",
        default="arch-derived",
        help="R2 bucket name for derived Arch build artifacts.",
    )
    r2_parser.add_argument(
        "--wrangler-command",
        default="npx wrangler",
    ...
    derived_publish_parser = subparsers.add_parser(
        "publish-derived",
        help="Upload deterministic Arch build outputs to arch-derived R2",
    )
    ...
    derived_publish_parser.add_argument(
        "--build-id",
        help="Build ID. Defaults to the ID inferred from reports or arch.db.",
    )
    derived_publish_parser.add_argument(
        "--r2-bucket",
        default="arch-derived",
    ...
    mirror_export_parser = subparsers.add_parser(
        "export-db-tables",
        help="Export an Arch SQLite DB artifact to per-table JSONL files",
    )
    mirror_export_parser.add_argument(
        "--db",
        type=Path,
        required=True,
        help="SQLite Arch DB path to export",
    )
    mirror_export_parser.add_argument(
        "--out",
        type=Path,
    ...
    mirror_load_parser = subparsers.add_parser(
        ...
        help="Load exported Arch JSONL mirror files into Supabase/Postgres",
    )
    ...
    if args.command == "validate-source-cells":
        path = (
            FIXTURE_SOURCE_CELLS_PATH
            if args.fixture or not args.input
            else args.input
        )
    ...
    if args.command == "export-db-tables":
        report = export_arch_db_table_files(
            args.db,
            args.out,
            replace=args.replace,
        )
