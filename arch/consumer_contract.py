"""Arch-owned downstream consumer contract exports.

The rows produced here are source-fact contract rows, not Microplex target rows.
They expose stable Arch identity and audit fields that downstream adapters can
consume without importing Arch internals or depending on source table layout.
"""
...
from arch.store import fact_to_mapping

CONSUMER_FACT_SCHEMA_VERSION = "arch.consumer_fact.v1"


@dataclass(frozen=True)
class ConsumerFactExportReport:
    """Counts from exporting Arch facts to consumer-contract JSONL."""
...
def validate_consumer_fact_contract(
    ...
    for index, fact in enumerate(facts):
        ...
        if filter_constraints and not fact.constraints:
            ...
            for constraint_payload, expected_count in (
                expected_constraint_payloads.items()
            ):
...
def _source_filter_variables(
    ...
    """Return source-specific variables that Arch can map from filters."""
    return frozenset(
        constraint.variable
        for constraint in filter_constraints
        if _is_source_specific_variable(fact, constraint.variable)
...
def _concept_leaf(concept: str) -> str:
    ...
    return concept.rsplit("#", maxsplit=1)[-1].rsplit(
        "/",
        maxsplit=1,
    )[-1].rsplit(".", maxsplit=1)[-1].rsplit(":", maxsplit=1)[-1]
