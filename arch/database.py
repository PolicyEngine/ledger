"""Build a relational Arch database artifact.

The database is an Arch-owned query surface for source-backed aggregate facts.
...
from arch.core import AggregateFact, build_aggregate_constraints, build_fact_key
...
class ArchDbBuildReport:
    """Counts from building a relational Arch DB artifact."""
...
def build_arch_db(
    ...
    *,
    ...
    """Build a deterministic SQLite Arch database artifact."""
    path = Path(db_path)
    if path.exists():
        if not replace:
            raise FileExistsError(f"Arch DB already exists: {path}")
        path.unlink()
...
def _create_schema(connection: sqlite3.Connection) -> None:
    connection.executescript(
        ...
        CREATE TABLE build_artifacts (
            ...
            build_id TEXT NOT NULL REFERENCES arch_builds(build_id),
...
def _build_id(
    ...
    _update_build_hash(digest, "schema", {"version": ARCH_DB_SCHEMA_VERSION})
    ...
    return f"arch.build.v1:{digest.hexdigest()[:24]}"
