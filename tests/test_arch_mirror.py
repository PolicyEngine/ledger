"""Tests for hosted-mirror exports from Arch DB artifacts."""
...
from arch.mirror import ARCH_MIRROR_TABLES, export_arch_db_tables, load_supabase_mirror
...
def test_export_arch_db_tables_writes_jsonl_and_manifest(tmp_path):
    ...
    report = export_arch_db_tables(db_path, output_dir)
    manifest = json.loads((output_dir / "manifest.json").read_text())

    assert report.table_count == len(ARCH_MIRROR_TABLES)
    assert manifest["table_count"] == len(ARCH_MIRROR_TABLES)
    assert {table["table"] for table in manifest["tables"]} == set(ARCH_MIRROR_TABLES)
...
def test_export_arch_db_tables_orders_rows_deterministically(tmp_path):
    db_path = tmp_path / "arch.db"
    ...
    first_report = export_arch_db_tables(db_path, first_output_dir)
    second_report = export_arch_db_tables(db_path, second_output_dir)
...
def test_export_db_tables_cli_emits_manifest_summary(tmp_path, capsys):
    ...
    assert payload["table_count"] == len(ARCH_MIRROR_TABLES)
...
def test_load_supabase_mirror_uses_schema_upserts_and_build_artifact_override(
    ...
    build_artifacts_path.write_text(
        json.dumps(
            {
                "build_artifact_key": "arch.build_artifact.v1:test",
                "build_id": "arch.build.v1:test",
                "artifact_kind": "json",
                "artifact_name": "reports/build_summary.json",
                "sha256": "abc",
                "size_bytes": 3,
                "r2_bucket": "arch-derived",
                "r2_key": "derived/test",
