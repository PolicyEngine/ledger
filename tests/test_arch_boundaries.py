"""Boundary tests for Arch core independence."""
...
def test_arch_modules_do_not_import_microplex_runtime():
    arch_root = Path(__file__).resolve().parents[1] / "arch"
    violations: list[str] = []

    for path in sorted(arch_root.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            for root in imported_roots:
                if root in FORBIDDEN_RUNTIME_IMPORTS:
                    relative_path = path.relative_to(arch_root.parent)
