"""Tests for Arch downstream consumer-contract exports."""
...
import arch.consumer_contract as consumer_contract
...
CONSUMER_FACT_SAMPLE_PATH = (
    Path(__file__).parents[1] / "arch" / "fixtures" / "consumer_facts.jsonl"
)
...
def test_consumer_fact_row_exposes_arch_and_lineage_keys():
    row = consumer_fact_row(_soi_agi_fact())

    assert row["schema_version"] == CONSUMER_FACT_SCHEMA_VERSION
    assert row["aggregate_fact_key"].startswith("arch.aggregate_fact.v2:")
    assert row["semantic_fact_key"].startswith("arch.semantic_fact.v2:")
    assert row["legacy_fact_key"].startswith("arch.fact.v1:")
    assert row["source_release_key"].startswith("arch.source_release.v2:")
    assert row["observed_measure_key"].startswith("arch.observed_measure.v2:")
...
def test_checked_in_consumer_fact_sample_matches_exporter():
    expected = [
        consumer_fact_row(fact)
        for fact in build_soi_table_1_1_facts(2023)[:3]
    ]
...
def test_export_consumer_facts_cli_writes_fixture(tmp_path, capsys):
    ...
    assert first_row["aggregate_fact_key"].startswith("arch.aggregate_fact.v2:")
