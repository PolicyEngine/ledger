"""Tests for source-backed Arch SOI fact builders."""
...
def test_build_soi_table_1_1_facts_includes_agi_brackets():
    ...
    facts_by_concept_and_range = {
        (fact.measure.concept, fact.filters["income_range"]): fact
        for fact in facts
    }
    ...
    agi = facts_by_concept_and_range[
        (AXIOM_IRC_AGI_CONCEPT, "100k_to_200k")
    ]
    tax = facts_by_concept_and_range[
        ("irs_soi.total_income_tax", "100k_to_200k")
    ]
...
def test_soi_facts_carry_source_cell_lineage():
    ...
    source_cell = next(
        cell
        for cell in build_soi_table_1_1_source_cells(2023)
        if cell.address == "D10"
    )
...
def test_build_soi_table_1_4_wage_facts_from_packaged_source():
    ...
    all_wages = facts_by_concept_and_range[
        ("us:statutes/26/62#input.wages", "all")
    ]
    ...
    ].value == 2_774_550_975_000
    assert facts_by_concept_and_range[
        ("irs_soi.taxable_net_capital_gains", "all")
    ].value == 966_168_014_000
    assert facts_by_concept_and_range[
        ("irs_soi.taxable_pension_income", "all")
    ].value == 932_130_236_000
    assert facts_by_concept_and_range[
...
def test_soi_table_1_4_source_region_spec_covers_wage_record_set():
    ...
    assert regions[0].left_column == 1
    assert regions[0].right_column == 89
