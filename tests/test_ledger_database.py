"""Tests for the relational Ledger database artifact."""
...
from ledger.core import build_aggregate_constraints
from ledger.jurisdictions.us.soi import AXIOM_IRC_AGI_CONCEPT
from ledger.database import build_ledger_db
from ledger.harness import build_ledger_db_file
...
def test_build_ledger_db_writes_aggregate_fact_constraints_and_lineage(tmp_path):
    db_path = tmp_path / "ledger-fixture.db"
    facts = build_soi_table_1_1_facts(2023)
    cells = build_soi_table_1_1_source_cells(2023)

    report = build_ledger_db(facts, db_path, source_cells=cells)
...
def test_build_ledger_db_build_id_changes_when_fact_payload_changes(tmp_path):
    cells = build_soi_table_1_1_source_cells(2023)
    facts = build_soi_table_1_1_facts(2023)
    changed_facts = [replace(facts[0], value=999), *facts[1:]]

    original = build_ledger_db(
        facts,
        tmp_path / "original.db",
        source_cells=cells,
    )
    changed = build_ledger_db(
        changed_facts,
        tmp_path / "changed.db",
        source_cells=cells,
    )
...
def test_build_ledger_db_rejects_unresolved_source_cell_lineage(tmp_path):
    fact = replace(
    ...
    with pytest.raises(sqlite3.IntegrityError):
        build_ledger_db([fact], tmp_path / "bad-lineage.db")


def test_build_ledger_db_file_uses_fixture_facts_and_cells(tmp_path):
    db_path = tmp_path / "ledger-fixture.db"

    report = build_ledger_db_file(db_path)
