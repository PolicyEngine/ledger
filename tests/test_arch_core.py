"""Tests for canonical Arch aggregate facts."""
...
def test_arch_core_imports_without_microplex(monkeypatch):
    ...
    def guarded_import(name, *args, **kwargs):
        if name == "microplex" or name.startswith("microplex."):
            raise AssertionError("arch.core imported microplex")
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", guarded_import)
    import arch.core

    importlib.reload(arch.core)
