# ADR: Ledger Fact Identity v2

Status: proposed after external ChatGPT Pro review on 2026-05-09

Implementation note: this ADR describes the Ledger identity model now used by
the `consumer_fact.v1` schema and generated `ledger.*` key prefixes.

## Decision

Ledger v2 uses layered identities instead of one overloaded target key:
...
3. `semantic_fact_key`
   ...
   This is the bridge for downstream reconciliation and Populace target
   selection, but it is not the primary Ledger source fact key.
...
Ledger is a source-data registry and build harness, not the Populace target
...
The v2 split keeps source facts stable while still giving Populace a clean
...
namespaces, for example `ledger:aggregate_fact:v2`.
...
If a publisher changes bytes under the same apparent vintage, Ledger creates a
...
- build timestamp;
- value;
- Populace target names, source-selection status, simulator aliases, aging,
  reconciliation, and activation state.

If two rows in the same source release produce the same `aggregate_fact_key`
with different values, Ledger should fail validation. That usually means a missing
...
Legacy direct target rows are compatibility inputs, not production Ledger facts.
...
Agents should not populate production Ledger facts at scale below `L2`.
