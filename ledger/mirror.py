"""Export deterministic Ledger DB artifacts for hosted mirrors."""
...
LEDGER_MIRROR_PRIMARY_KEYS = {
    "ledger_builds": "build_id",
...
@dataclass(frozen=True)
class LedgerMirrorExportReport:
    """Report for a hosted-mirror export from a local Ledger DB artifact."""

    db_path: str
    output_dir: str
    exported_at: str
    build_ids: tuple[str, ...]
    tables: tuple[LedgerTableExport, ...]
...
class SupabaseMirrorLoadReport:
    """Report from loading Ledger JSONL mirror files into Supabase/Postgres."""
...
def export_ledger_db_tables(
    db_path: str | Path,
    output_dir: str | Path,
    *,
    replace: bool = False,
) -> LedgerMirrorExportReport:
    """Export Ledger SQLite tables to JSONL files for bulk hosted loading."""
    db = Path(db_path)
    if not db.exists():
        raise FileNotFoundError(f"Ledger DB not found: {db}")

    output = Path(output_dir)
    _prepare_output_dir(output, replace=replace)

    exports: list[LedgerTableExport] = []
    with sqlite3.connect(db) as connection:
        connection.row_factory = sqlite3.Row
        build_ids = tuple(
            row["build_id"]
            for row in connection.execute(
                "SELECT build_id FROM ledger_builds ORDER BY build_id"
            ).fetchall()
        )
        for table in LEDGER_MIRROR_TABLES:
            exports.append(_export_table(connection, output, table))

    report = LedgerMirrorExportReport(
        db_path=str(db),
    ...
    *,
    ...
    for table in LEDGER_MIRROR_TABLES:
        ...
        try:
            ...
            continue
        manifest_row_count = manifest_counts.get(table) if uses_default_path else None
        row_count_matches_manifest = (
            row_count == manifest_row_count if manifest_row_count is not None else None
        )
...
def _export_table(
    ...
    with path.open("w", encoding="utf-8") as file:
        rows = connection.execute(
            f"SELECT * FROM {table} ORDER BY {LEDGER_MIRROR_PRIMARY_KEYS[table]}"
        ).fetchall()
        for row in rows:
            payload = {key: _export_value(key, row[key]) for key in row.keys()}
            file.write(json.dumps(payload, sort_keys=True))
            file.write("\n")
            row_count += 1

    return LedgerTableExport(
        table=table,
        path=str(path),
        row_count=row_count,
...
def _upsert_supabase_batch(
    ...
    *,
    ...
    query.upsert(rows, on_conflict=LEDGER_MIRROR_PRIMARY_KEYS[table]).execute()
