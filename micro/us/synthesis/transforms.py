"""
Data transformations for tax variables.
...
class ZeroInflatedTransform:
    ...
    def combine(self, indicator: np.ndarray, positive_values: np.ndarray) -> np.ndarray:
        ...
        positive_mask = indicator > 0.5  # Threshold at 0.5
...
class LogTransform:
    ...
    def __init__(self, offset: float = 0.0):
        ...
        Args:
            offset: Value to add before log (for handling zeros)
        """
        self.offset = offset
...
class Standardizer:
    ...
    def fit(self, x: np.ndarray, weights: np.ndarray) -> "Standardizer":
        """
...
class TaxVariableTransformer:
    """
    Complete transformation pipeline for a single tax variable.
    ...
    def fit(self, x: np.ndarray, weights: np.ndarray) -> "TaxVariableTransformer":
        """
    ...
    def transform(
        ...
        For zero-inflated variables, this returns transformed positive values
        at the positive positions, and NaN at zero positions (which will be
...
class MultiVariableTransformer:
    """Transform multiple tax variables at once."""
    ...
    def __init__(self, var_names: list):
        ...
        self.transformers_: Dict[str, TaxVariableTransformer] = {}

    def fit(self, data: Dict[str, np.ndarray]) -> "MultiVariableTransformer":
        """
        Fit transformers for all variables.

        Args:
            data: Dict with variable names as keys, arrays as values.
                  Must also include 'weight' key.
